...
export function create_msg(msg) {
    ...
    author.style = `display: block; margin-bottom: 7px; font-size: 14.5px; ${msg.author == getCookie("alias") ? "text-align: right; color: #fff;" : "color: var(--secondary);"}`;
    author.innerText = msg.author == getCookie("alias") ? "me" : msg.author;

    // message box
    const div = document.createElement("div");
    div.className = "message";
    div.style = `${msg.author == getCookie("alias") ? "background-color: var(--primary); border-color: var(--primary-darker); margin-left: auto;" : "color: var(--secondary);"}`;
